...
log using "\\Client\H$\Dropbox (University of Michigan)\R03 WEST\do\NSFG\ANALYSIS\BIVAR\NSFG_FEMALE_1519_BIVAR.log", replace
...
svy, subpop(if age_eligible == 1): tab freqBINGE IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab freqBINGE IDENTITY_B, col ci obs
...
svy, subpop(if age_eligible == 1 & pregnancy_eligible == 1): tab pregnancy IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1 & pregnancy_eligible == 1): tab pregnancy IDENTITY_B, col ci obs

*Life Contraceptive use

svy, subpop(if age_eligible == 1): tab ever_contraceptive IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab ever_contraceptive IDENTITY_B, col ci obs

*Contraceptive use per year

svy, subpop(if age_eligible == 1): tab pycontraceptive IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab pycontraceptive IDENTITY_B, col ci obs
...
svy, subpop(if age_eligible == 1): tab py_sex_nocontra IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab py_sex_nocontra IDENTITY_B, col ci obs
...
svy, subpop(if age_eligible == 1): tab race IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab race IDENTITY_B, col ci obs

*Education (Highest degree)

svy, subpop(if age_eligible == 1): tab cateduc IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab cateduc IDENTITY_B, col ci obs

*Total Family Income

svy, subpop(if age_eligible == 1): tab faminc IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab faminc IDENTITY_B, col ci obs

*Hispanic

svy, subpop(if age_eligible == 1): tab hispanic IDENTITY_A, col ci obs
svy, subpop(if age_eligible == 1): tab hispanic IDENTITY_A, col ci obs
